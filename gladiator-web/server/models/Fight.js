...
FightSchema.methods.addTurns = async function () {
  ...
  for (
    let turnIndex = 0;
    turnIndex < this.turnsPerRound * this.rounds;
    turnIndex++
  ) {
    ...
    if (sequencialMap.has(selectedFighter._id)) {
      sequencialMap.set(
        selectedFighter._id,
        sequencialMap.get(selectedFighter._id) + 1
      );
    } else {
      sequencialMap.clear();
...
FightSchema.methods.simulate = async function () {
    ...
    await this.addTurns();
    console.log("Turns Added", this.turns.length);
    ...
    this.turns.forEach(async (turn, index) => {
        ...
        const attackerPosition = this.arena.fightFloor.getFighterCords(
        attacker._id.toString()
        ).cords;

        const possibleCellsToMoveTo = this.arena.fightFloor.getNeighboringCells(
        attackerPosition.x,
        attackerPosition.y
        );
        ...
        console.log("Moved to: ", turn.moveTo.cords)
        
        this.fightReplay.push(this.arena.fightFloor);
        console.log("current replay: ", this.fightReplay)

        const moveOptions = await attacker.movesInRangeOfAnotherFighter(
        selectedCell.cords,
        this.arena.fightFloor.grid
        );

        console.log("Moves in Range: ", moveOptions.length );
        ...
        if (moveOptions.length > 0) {
            ...
            const selectedStrikingLimb = getRandomElementFromArray(
                combatSkill.moveStatistics.move.strikingLimb
            );
            
            const selectedTargetLimb = getRandomElementFromArray(
                combatSkill.moveStatistics.move.targets
            );
            ...
            //build defense
            const opponent = this.fighters.find(
                (fighter) => fighter._id.toString() === opponentId
            );

            // Update opponent values
            const {
                combatSkill: targetCombatSkill,
                strikingLimb: targetStrikingLimb,
                target: targetMovetarget,
                pattern: targetPattern,
            } = opponent.autoSelectDefensiveCombatSkill();
...
function buildAttack(movePatternMappings) {
  const strike = currentFighter.autoSelectAttack(
    movePatternMappings.map((item) => item.combatSkills).flat()
  );
...
async function buildDefense() {
  currentFightTurn.action.defense =
    await opponent.autoSelectDefensiveCombatSkill();
}
